...
print("[KLOSO ARSENAL] Services Loaded.")

-- [[ THEME & SETTINGS ]]
local Theme = {
	Accent = Color3.fromRGB(255, 60, 60), -- Red accent for Arsenal
	Bg = Color3.fromRGB(15, 12, 12),
	Card = Color3.fromRGB(25, 20, 20),
	Border = Color3.fromRGB(60, 30, 30),
	Text = Color3.fromRGB(240, 240, 250),
	Sub = Color3.fromRGB(180, 150, 150)
}
...
local function GetESPObj(p)
    if not ESP_Objects[p] then
        ESP_Objects[p] = {
            ...
            Health = Drawing.new("Line"), HealthOutline = Drawing.new("Line")
        }
...
Connections.RenderLoop = RS.RenderStepped:Connect(function()
    ...
    if Settings.Combat.Crosshair then
        ...
        local s = 8
        CrosshairX.From = center - Vector2.new(s, 0) CrosshairX.To = center + Vector2.new(s, 0)
        CrosshairY.From = center - Vector2.new(0, s) CrosshairY.To = center + Vector2.new(0, s)
    ...
    for _, p in pairs(Players:GetPlayers()) do
        ...
        if onScreen then
            ...
            local color = Color3.new(1, 0.2, 0.2) -- Enemies are red
...
local Gui = Create("ScreenGui", {Parent = (gethui or function() return CoreGui end)(), Name = "KlosoArsenal"})
...
local Title = Create("TextLabel", {Parent = Sidebar, Size = UDim2.new(1, 0, 0, 50), BackgroundTransparency = 1, Text = "KLOSO ARSENAL", TextColor3 = Theme.Accent, TextSize = 14, Font = Enum.Font.GothamBold})
...
Toggle(CombatPage, "Team Check (Arsenal)", true, function(v) Settings.Combat.TeamCheck = v end)
Toggle(CombatPage, "Wall Check", true, function(v) Settings.Combat.WallCheck = v end)
Toggle(CombatPage, "Custom Crosshair", false, function(v) Settings.Combat.Crosshair = v end)
Slider(CombatPage, "FOV Radius", 30, 800, 100, function(v) Settings.Combat.FOV = v end)
...
Slider(PlayerPage, "Speed Value", 16, 250, 16, function(v) Settings.Movement.SpeedVal = v end)
...
Slider(SettingsPage, "Accent R", 0, 255, 255, function(v) Theme.Accent = Color3.fromRGB(v, Theme.Accent.G * 255, Theme.Accent.B * 255) Title.TextColor3 = Theme.Accent FOVCircle.Color = Theme.Accent CrosshairX.Color = Theme.Accent CrosshairY.Color = Theme.Accent end)
Slider(SettingsPage, "Accent G", 0, 255, 60, function(v) Theme.Accent = Color3.fromRGB(Theme.Accent.R * 255, v, Theme.Accent.B * 255) Title.TextColor3 = Theme.Accent FOVCircle.Color = Theme.Accent CrosshairX.Color = Theme.Accent CrosshairY.Color = Theme.Accent end)
Slider(SettingsPage, "Accent B", 0, 255, 60, function(v) Theme.Accent = Color3.fromRGB(Theme.Accent.R * 255, Theme.Accent.G * 255, v) Title.TextColor3 = Theme.Accent FOVCircle.Color = Theme.Accent CrosshairX.Color = Theme.Accent CrosshairY.Color = Theme.Accent end)
...
print("[KLOSO ARSENAL] Initialization Complete.")
